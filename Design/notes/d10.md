...
Think of a YouTube channel. When you subscribe to a channel, you become an "observer." Whenever the channel (the "subject") uploads a new video, all subscribers get notified. You don't have to keep checking the channel manually—you're automatically informed when there's something new.
...
- **Loose Coupling:** When you want the subject and observers to be independent—the subject doesn't need to know the concrete classes of its observers.
...
class StockExchange implements Subject {
  ...
  setStockPrice(symbol: string, price: number): void {
    console.log(`\n📈 Stock Update: ${symbol} is now $${price}`);
...
class MobileAppDisplay implements Observer {
  ...
  update(stockSymbol: string, price: number): void {
    console.log(
      `📱 [${this.name}] Push notification: ${stockSymbol} = $${price}`
    );
  }
}
...
class WebDashboard implements Observer {
  update(stockSymbol: string, price: number): void {
    console.log(
      `🖥️  [Web Dashboard] Updating chart: ${stockSymbol} at $${price}`
    );
  }
}
...
class TradingBot implements Observer {
  ...
  update(stockSymbol: string, price: number): void {
    if (price < this.threshold) {
      console.log(
        `🤖 [Trading Bot] BUY SIGNAL! ${stockSymbol} at $${price} is below threshold $${this.threshold}`
      );
    } else {
      console.log(`🤖 [Trading Bot] Holding. ${stockSymbol} at $${price}`);
    }
  }
}
...
### Real-World Analogy for Strategy Pattern
...
### Key Features & Concepts of the Strategy Pattern
...
In Express middleware, you might use different authentication strategies (JWT, OAuth, API Key). The passport.js library is a great example—it uses the Strategy pattern to support 500+ authentication strategies through a unified interface.
...
class CreditCardPayment implements PaymentStrategy {
  ...
  pay(amount: number): boolean {
    ...
    console.log(
      `💳 Processing $${amount} via Credit Card ending in ${lastFour}`
    );
...
class CryptoPayment implements PaymentStrategy {
  ...
  pay(amount: number): boolean {
    console.log(`₿  Processing $${amount} in ${this.cryptoType}`);
...
class ShoppingCart {
  ...
  checkout(): boolean {
    if (!this.paymentStrategy) {
      console.log("❌ Error: Please select a payment method first!");
      return false;
    }
...
4. In modern development:
   - **Observer** → RxJS, Event Emitters, Pub/Sub systems, WebSockets
   - **Strategy** → Dependency Injection, Plugin architectures, Configuration-based behavior
